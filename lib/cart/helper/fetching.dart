...
Future<Product> fetchProductDetail(
  ...
  // web: 127.0.0.1
  final response =
      await request.get('http://muhammad-rafli33-souvenirkita.pbp.cs.ui.ac.id/adminview/json/$productId/');
...
Future<void> addCertainCartProduct(CookieRequest request) async {
  // web: 127.0.0.1
  await request.get('http://muhammad-rafli33-souvenirkita.pbp.cs.ui.ac.id/cart/inc_amount/d2532d21-2cf1-4df7-95f2-b9a262ff9e56/');
}

Future<List<CartProduct>> fetchCartProduct(CookieRequest request) async {
  // web: 127.0.0.1
  final response = await request.get('http://muhammad-rafli33-souvenirkita.pbp.cs.ui.ac.id/cart/user-cart-products/');
